...
public class DataBaseMember extends DataBase {
    ...
    public Member getStudentFromBase(String Login){
        ...
        try {
            this.connect();
            sqlStmt = "SELECT usernama,password,fname,lname, "
                    + "age,sex,telephone,email,university,department,city, "
                    + "address,zip,min_price "
                    + "FROM `UnannyDB`.`Babysitter` WHERE username = '"
                    + Login + "'";
            ResultSet res = statement.executeQuery(sqlStmt);
            if(res.next()){
                newStudent.setUserName(res.getString("usernama"));
                newStudent.setFirstName(res.getString("fame"));
                newStudent.setLastName(res.getString("lame"));
                newStudent.setUniversity(res.getString("university"));
                newStudent.setDepartment(res.getString("department"));
                newStudent.setGender(res.getString("sex"));
                newStudent.setCity(res.getString("city"));
