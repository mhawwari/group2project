...
public class Controller extends HttpServlet {
    ...
    public Controller(){
        ...
        dispatchTable.put(ServletCommands.param_CNTRL_Gotopost, new Handler_Post());
...
protected void handle(HttpServletRequest request, HttpServletResponse response)
    ...
    private class CommonOperations{
        
       
        public void setCommon(HttpServletRequest request, HttpServletResponse response){
            String userName = (String)session.getAttribute(ServletCommands.attrUserName);
            if(userName!=null){
                DataBaseMember connector = new DataBaseMember();
                Member student = connector.getStudentFromBase(userName);
                request.setAttribute(ServletCommands.attrUserName, userName);
                request.setAttribute(ServletCommands.attrUserObject, student);
            }
        }
    }
     ...
     private class Handler_Login implements Dispatcher {
        ...
        public void execute(HttpServletRequest request, HttpServletResponse response){
           ...
           if(loginState){
               ...
               co.setCommon(request, response);
               session.setAttribute(ServletCommands.attrUserName, userName);  
               forward("/home_nanny.html",request,response);
           }
           else{
    ...
    private class Handler_Post implements Dispatcher {
        @Override
        public void execute(HttpServletRequest request, HttpServletResponse response){
            session.removeAttribute(ServletCommands.attrUserName);
            request.setAttribute(ServletCommands.attrUserName, null);
            forward("/test.html",request,response);
        }
    }
